...
serve(async (req) => {
  ...
  try {
    const { drama_id, title, ui_language = "ko-KR", force_refresh = false } = await req.json();

    if (!drama_id || !title) {
      return new Response(
        JSON.stringify({ error: "drama_id and title are required" }),
    ...
    if (!force_refresh) {
      const { data: cached } = await supabase
        .from("drama_sentiment_cache")
        .select("data")
        .eq("drama_id", String(drama_id))
        .eq("language", ui_language)
