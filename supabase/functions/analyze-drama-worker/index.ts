...
async function runAnalysis(
  ...
  try {
    await updateJob(supabase, jobId, {
      ...
      stage_message: "YouTube 드라마 리뷰 검색 중...",
      progress: 10,
    });

    const ytComments = await (ytKey
      ? searchYouTubeVideos(`${title} 드라마 리뷰`, ytKey)
    ...
    await updateJob(supabase, jobId, {
      ...
      stage_message: `데이터 수집 완료 (YouTube ${ytComments.length}개)`,
      progress: 40,
    });

    const allTexts = [...ytComments];
    if (allTexts.length < 3) {
      allTexts.push(`${title} 드라마에 대한 분석`);
    }
    ...
    const koreanHint = hasKorean
      ? `\nKorean drama slang notes:
- "연기력" = acting skill, "개연성" = plot coherence, "OST" = original soundtrack
- "ㅋㅋ"/"ㅎㅎ" = laughter/positive, "ㅠㅠ"/"ㅜㅜ" = emotional/moving
- "레전드" = legendary/masterpiece, "명작" = classic, "역대급" = best ever
- "용두사미" = strong start weak ending, "막장" = over-the-top/makjang drama
- "감동" = moving/emotional, "힐링" = healing/soothing, "현실고증" = realistic portrayal`
      : "";

    const prompt = `You are a drama review analyst. Analyze the following reviews about the drama "${title}".
${koreanHint}
REVIEWS (${uniqueComments.length} total, sources: YouTube comments):
...
Return ONLY a valid JSON object with this exact structure:
{
  ...
  "viewing_point": "<2-3 sentences about why audiences enjoyed this drama>",
...
- positive_keywords: up to 5 praised aspects like acting, story, cinematography, OST, emotion (in ${outputLang})
...
- Respond ONLY with the JSON object`;
    ...
    const result = {
      ...
      data_sources: { youtube_comments: ytComments.length },
    };

    await supabase.from("drama_sentiment_cache").upsert(
      { id: `${dramaId}:${uiLanguage}`, drama_id: dramaId, language: uiLanguage, data: result, expires_at: new Date(Date.now() + CACHE_TTL_MS).toISOString() },
      { onConflict: "id" }
    );
    ...
    console.error("analyze-drama-worker error:", message);
...
serve(async (req) => {
  ...
  const { job_id, drama_id, title, ui_language = "ko-KR" } = body;

  if (!job_id || !drama_id || !title) {
    return new Response(JSON.stringify({ error: "job_id, drama_id, title required" }), {
      status: 400,
      headers: CORS,
    });
  }
  ...
  const runPromise = runAnalysis(supabase, job_id, String(drama_id), title, ui_language);
