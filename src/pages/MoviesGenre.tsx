...
export default function MoviesGenre() {
  ...
  const { data, isLoading, isError } = useQuery<DiscoverResponse>({
    queryKey: ["movies-genre", genres, uiLanguage, page],
    queryFn: async () => {
      const { data, error } = await supabase.functions.invoke("discover-movies", {
        body: { genres, language: uiLanguage, sort_by: "popularity.desc", min_vote_count: 100, page },
      });
  ...
  function goPage(p: number) {
    navigate(`/movies/genre?genres=${genres}&label=${encodeURIComponent(label)}&page=${p}`);
  }
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 space-y-6">
      ...
      {!isLoading && !isError && data?.results?.length === 0 && <EmptyState type="search" />}
