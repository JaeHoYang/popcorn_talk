...
const SORT_OPTIONS = [
  { value: "popularity.desc",    ko: "인기순",   en: "Most Popular" },
  { value: "vote_average.desc",  ko: "평점순",   en: "Highest Rated" },
  { value: "release_date.desc",  ko: "최신순",   en: "Newest" },
  { value: "revenue.desc",       ko: "흥행순",   en: "Revenue" },
];
...
export default function AdvancedFilter({ onApply }: Props) {
  ...
  const reset  = () => { setFilters(DEFAULT_FILTERS); onApply(DEFAULT_FILTERS); setOpen(false); };
    ...
    <div className="relative">
      ...
      {open && (
        <>
          ...
          <div className="absolute top-full mt-2 left-0 w-80 bg-slate-800 border border-slate-700 rounded-xl shadow-2xl z-50 p-5 space-y-5">
            ...
            {/* 개봉연도 */}
            <div>
              <p className="text-xs font-semibold text-slate-400 mb-2">{t("개봉연도", "Release Year")}</p>
            ...
            {/* 원본 언어 */}
            <div>
              <p className="text-xs font-semibold text-slate-400 mb-2">{t("원본 언어", "Original Language")}</p>
              <div className="flex flex-wrap gap-1.5">
                {LANG_OPTIONS.map((opt) => (
                  <button
                    key={opt.value}
                    onClick={() => setFilters((f) => ({ ...f, originalLanguage: opt.value }))}
                    className={cn(
                      "px-3 py-1 text-xs rounded-full border transition-colors",
                      filters.originalLanguage === opt.value
                        ? "bg-blue-500 border-blue-500 text-white"
                        : "border-slate-600 text-slate-300 hover:border-blue-400"
                    )}
                  >
